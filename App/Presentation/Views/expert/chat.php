...
$farmerName = htmlspecialchars($farmer ? $farmer->getUsername() : 'Farmer');
$images = $images ?? [];
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Expert Consultation - Grape Cultivation Advisory</title>
...
<div class="h-screen flex flex-col bg-slate-50 overflow-hidden">
    ...
    <header class="h-16 border-b border-slate-200 bg-white px-4 md:px-6 flex items-center justify-between shrink-0 shadow-sm z-10">
        <div class="flex items-center space-x-3">
            ...
            <div>
                ...
                <p class="text-[10px] text-slate-500 font-medium">Chatting with <strong><?= $farmerName ?></strong></p>
            </div>
        </div>
        <div class="flex items-center space-x-2">
            ...
            <a href="<?= BASE_URL ?>/expert/consultations/view?id=<?= $consultationId ?>" class="h-9 w-9 flex items-center justify-center rounded-xl border border-slate-200 hover:bg-slate-50 text-slate-400 hover:text-slate-600 transition-colors">
    ...
    <!-- Attached Images Bar -->
    <?php if (!empty($images)): ?>
    <div class="px-6 py-3 bg-slate-50 border-b border-slate-200 flex gap-2 overflow-x-auto shrink-0">
        <span class="text-[10px] font-bold text-slate-400 uppercase tracking-wider self-center mr-2">Images:</span>
        <?php foreach ($images as $img): ?>
        <a href="<?= BASE_URL . '/' . htmlspecialchars($img['image_path']) ?>" target="_blank">
            <img src="<?= BASE_URL . '/' . htmlspecialchars($img['image_path']) ?>" class="w-12 h-12 rounded-lg object-cover border border-slate-200 hover:opacity-80 transition-opacity" alt="Attached image">
        </a>
        <?php endforeach; ?>
    </div>
    <?php endif; ?>
    ...
    <div class="flex-1 flex overflow-hidden">
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
            ...
            <div id="typing-indicator" class="hidden px-6 py-2 bg-white border-t border-slate-100 flex items-center space-x-2">
                ...
                <span class="text-xs text-slate-400">Farmer is typing...</span>
            </div>
            ...
            <div class="p-4 bg-white border-t border-slate-200 shrink-0">
                <form id="chat-form" class="flex items-center space-x-2">
                    <input type="text" id="message-input" placeholder="Type your expert advice for the farmer..." autofocus
                ...
                <div class="flex justify-between items-center px-1 mt-1.5 text-[9px] text-slate-400">
                    <span><i class="fa-solid fa-leaf text-emerald-500 mr-1"></i> Grape Cultivation Expert Panel</span>
            ...
            div.innerHTML = `
                <div class="max-w-[80%]" ${idAttr}>
                    <div class="flex items-center gap-2 mb-1">
                        <div class="w-6 h-6 rounded-lg bg-gradient-to-br from-amber-500 to-orange-600 flex items-center justify-center text-white text-[8px] font-bold">F</div>
