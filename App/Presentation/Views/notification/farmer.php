                ...
                <tbody>
                    ...
                    <tr class="border-b border-slate-50 hover:bg-slate-50 transition-colors <?= !$notif->isRead() ? 'bg-emerald-50/30' : '' ?>">
                        <td class="px-4 py-3 w-12">
                            <span class="inline-flex items-center justify-center w-8 h-8 rounded-lg shrink-0 <?= !$notif->isRead() ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-500' ?>">
                                <?php if ($notif->getType() === 'consultation_assigned'): ?>
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                                <?php elseif ($notif->getType() === 'message_received' || $notif->getType() === 'new_message'): ?>
                                ...
                                <?php elseif ($notif->getType() === 'admin_action' || $notif->getType() === 'profile_update'): ?>
                        ...
                        <td class="px-4 py-3">
                            ...
                            <a href="<?= $notifHref ?>" class="block group">
                                ...
                                <span class="text-[10px] text-slate-400 mt-0.5 block"><?= $notif->getCreatedAt()->format('M d, Y h:i A') ?></span>
                            </a>
                        </td>
